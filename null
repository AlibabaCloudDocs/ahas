...
## 接入Agent之后，AHAS控制台找不到应用？

排查步骤如下。

1.  检查应用是否是EDAS应用，如果是EDAS应用必须用Admin账号接入Agent，否则将无法接入成功。

2.  查看日志/home/admin/logs/csp/sentinel-record.log是否有报错，根据报错内容进行修改。

3.  如果日志或者目录不存在，则执行`java -version`查看内容。
    ...
    2.  执行`java -version`。
        ...
        -   若输出结果输出Agent相关信息，则表示Agent接入成功，可以看到执行的Agent的内容，如下图所示。

            ![FAQ Agent2](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/1742942061/p172200.png)

        -   若输出结果出现如下报错，则表示`namespance`不存在，请在[AHAS控制台](https://ahas.console.aliyun.com)相应的Region下创建命令`namespace`下的环境或修改命令`namespace`为其他环境（例如default）。
...
AHAS控制台的应用防护页面显示应用接入成功，但是没有数据显示，可能为以下2个原因：
